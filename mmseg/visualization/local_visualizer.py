...
@VISUALIZERS.register_module()
class TestSegLocalVisualizer(Visualizer):
    """Local Visualizer.
    ...
    def add_datasample(
            ...
            image: np.ndarray,
            data_sample: Optional[SegDataSample] = None,
            draw_gt: bool = False, # changed to False to save only prediction
            draw_pred: bool = True,
            show: bool = False,
            wait_time: float = 0,
            # TODO: Supported in mmengine's Viusalizer.
            out_file: Optional[str] = None,
            step: int = 0,
            with_labels: Optional[bool] = False) -> None: # changed to False for later blending with input image 
        """Draw datasample and save to all backends.
